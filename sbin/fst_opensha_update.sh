...
echo "Checking for updates to OpenSHA. You can disable these checks by setting the environmental variable OPENSHA_JAR_DISABLE_UPDATE=1"

DOWNLOAD="${OPENSHA_JAR_FORCE_DOWNLOAD:-0}"
REBUILD="${OPENSHA_JAR_FORCE_REBUILD:-0}"
UPDAYS="${OPENSHA_JAR_UPDATE_DAYS:-7}"
if [[ $UPDAYS -lt 1 ]];then
	UPDAYS=1
fi
...
if [[ ! -e ./git && ! -e $DIR/$JAR ]];then
	...
	if [[ $RET -ne 0 ]];then
		echo "We will instead download a nightly build and update every $UPDAYS days. Change update frequency by setting OPENSHA_JAR_UPDATE_DAYS.";
		DOWNLOAD=1
	else
...
#			echo "remote: $REMOTE_HASH"
		...
		if [[ $UP_TO_DATE -ne 1 ]];then
			...
			case "$response" in
				[nN][oO]|[nN]) 
					DOWNLOAD=0
					echo "Skipping jar download, will check again in $UPDAYS days. You can change the frequency of this check by setting OPENSHA_JAR_UPDATE_DAYS."
					touch $DIR/$JAR
					;;
				*)
					DOWNLOAD=1
			...
			if [[ -e $DIR/build-version.githash && $(find "$DIR/$JAR" -mtime +$UPDAYS -print) ]]; then
				...
				if [ $LOCAL != $CUR_HASH ];then
					...
					case "$response" in
							...
							echo "Skipping jar rebuild, will prompt again in $UPDAYS days or next time the upstream repository is updated. You can change the frequency of this check by setting OPENSHA_JAR_UPDATE_DAYS."
							touch $DIR/$JAR
							;;
						*)
							REBUILD=1
